...
$sitecode = 'mlo';
    ...
    for($i=0;$i<$arylen;$i++){
        ...
        if($numrows == 0){
            $myquery = "INSERT INTO climate_co2_data2(sitecode, co2_value, timestamp_co2_recorded) VALUES('".$a_data[$i]['sitecode']."', '".$a_data[$i]['co2_value']."', '".$a_data[$i]['timestamp_co2_recorded']."')";
            $query = mysql_query($myquery);
            echo "Added ".date('Y-m-d H:m:i',$a_data[$i]['timestamp_co2_recorded'])." - ".$a_data[$i]['co2_value']." for ".$a_data[$i]['sitecode']." to db.\r\n";
        } else {
            echo "Value already exists on ".date('Y-m-d H:m:i',$a_data[$i]['timestamp_co2_recorded'])." for ".$a_data[$i]['sitecode'].".\r\n";
        }
        if ( ! $query ) {
            echo mysql_error();
            die;
